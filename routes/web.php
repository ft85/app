...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone', 'AdminSidebarMenu', 'CheckUserLogin'])->group(function () {
    ...
    Route::post('/verify-tax-number', [ContactController::class, 'verifyTaxNumber'])->name('verify.tax_number'); //VERIFY TIN
 ...
 //   Route::resource('rraimports', RRAController::class)->except(['index']);

//    Route::get('/rra', [App\Http\Controllers\RRAController::class, 'index'])->name('rra.index');

    Route::get('/rra', [RRAController::class, 'index'])->name('rra.index');
    

// Fetch the DataTables data (e.g., for AJAX)
   Route::get('/rra/data', [RRAController::class, 'getData'])->name('rra.data');

   // Route in web.php
   Route::get('/rra/line-details/{id}', [RRAController::class, 'getLineDetails']);

   Route::post('/rra/add-stock', [RRAController::class, 'addStock']);


   


   Route::post('/rra/syncimports', [RRAController::class, 'syncimports']);

  // Route::post('/syncimports', [RRAController::class, 'syncimports'])->name('sync.imports');

 // reg functionalities
   Route::get('/reg', [RegController::class, 'index'])->name('reg.index');

   Route::post('/verify-meter', [RegController::class, 'verifyMeter'])->name('verify.meter'); ////VERIFY METER NUMBER

   Route::get('/copy', [RegController::class, 'index2'])->name('reg.index2');

   Route::post('/verify-copy', [RegController::class, 'verifycopy'])->name('verify.copy'); //  VERIFY Payment copy

   Route::get('/mhistory', [RegController::class, 'index3'])->name('reg.index3');

   Route::post('/meterhistory', [RegController::class, 'meterstatement'])->name('meter.history'); //  VERIFY Payment copy

   
   Route::get('/vhistory', [RegController::class, 'index4'])->name('reg.index4');

   Route::post('/vendorhistory', [RegController::class, 'accountstatement'])->name('vendor.history'); //  VERIFY Payment copy

   Route::get('/vsummary', [RegController::class, 'index5'])->name('reg.index5');

   Route::post('/vendorsummary', [RegController::class, 'vendorsummary'])->name('vendor.summary'); //  VERIFY Payment copy

   Route::get('/password', [RegController::class, 'index6'])->name('reg.index6');

   Route::get('/retry', [RegController::class, 'index7'])->name('reg.index7');

   Route::post('/verify-retry', [RegController::class, 'verifyretry'])->name('verify.retry'); //  VERIFY Payment copy

   Route::get('/pdf-template', [RegController::class, 'pdfTemplate'])->name('pdf.template');

   Route::get('/reg/print/{id}', [RegController::class, 'printToken'])->name('reg.print');

  
   Route::post('/changepass', [RegController::class, 'changepass'])->name('change.pass'); //  change password eucl 

   Route::get('/listvends', [RegController::class, 'list'])->name('reg.list');

   Route::post('/payment', [RegController::class, 'paymentconfirmation'])->name('payment.confirm'); //  change password eucl 

   Route::get('/payment/data', [RegController::class, 'getpaymentdata'])->name('reg.data');

   Route::get('/listbalances', [RegController::class, 'list2'])->name('reg.list2');


   Route::get('/listbalances/all', [RegController::class, 'getBalanceData'])->name('reg.data2');

   //Route::get('/liststatment', [RegController::class, 'list3'])->name('reg.list3');


 //  Route::get('/liststatment/{id}', [RegController::class, 'getstatement'])->name('reg.list3');

   Route::get('/liststatment/{contactId}', [RegController::class, 'getstatement'])->name('reg.list3');


  

   Route::post('/loadcredit', [RegController::class, 'saveCredit'])->name('reg.savecredit');
    ...
    Route::prefix('modules')->group(function () {
        ...
        // Route::get('/print-line-order', [Restaurant\OrderController::class, 'printLineOrder']);
        Route::get('/print-line-order/{ids}', action: [Restaurant\OrderController::class, 'printLineOrder']);
...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone'])->group(function () {
    ...
    Route::get('/user-line-orders', [SellPosController::class, 'getUserLineOrders']);
    Route::get('/get-customer-copay/{id}', [SellPosController::class,'getCustomerCopay']);

    Route::get('/showButton', [SellPosController::class,'showButton']);
    
    
    
    
Route::get('/check-permission', function() {
    $permission = \Spatie\Permission\Models\Permission::where('name', 'location.31')->first();
    dd($permission); // This will dump the result
});
