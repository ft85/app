...
class ImportOpeningStockController extends Controller
{
    ...
    private function addOpeningStock($opening_stock, $product, $business_id, $unit_cost_before_tax, $transaction = null)
    {
        ...
        $business_locations = BusinessLocation::where('id', $opening_stock['location_id'])
        ->where('business_id', $business_id)
        ->first();

        $username = $business_locations->username;
        $product_info->id;
        $item_details = Product::find($product->id);
        ...
        "item_purchase_or_sale_price"=>$purchase_price_inc_tax,
        "item_purchase_or_sale_currency"=>'BIF',
        "item_cost_price"=>$purchase_price_inc_tax,
        "item_cost_price_currency"=>'BIF',
        "item_movement_type"=>'EI',
        "item_movement_description"=>'Opening Stock',
