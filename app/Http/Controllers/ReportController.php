...
class ReportController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            ->where(function ($q) use ($business_id, $contact_filter1, $contact_filter2, $parent_payment_query_part) {
                $q->whereRaw("(transaction_payments.transaction_id IS NOT NULL AND t.type IN ('sell', 'opening_balance') $parent_payment_query_part $contact_filter1)")
                    ->orWhereRaw("EXISTS(SELECT * FROM transaction_payments as tp 
                        ...
                        AND tp.parent_id=transaction_payments.id $contact_filter2)");
            })
            ->select(
                DB::raw("IF(transaction_payments.transaction_id IS NULL, 
                            (SELECT contacts.name FROM transactions as ts
                            JOIN contacts ON ts.contact_id=contacts.id 
                            WHERE ts.id=(
                                    SELECT tps.transaction_id FROM transaction_payments as tps
                                    WHERE tps.parent_id=transaction_payments.id LIMIT 1
                                )
                            ),
                            (SELECT CONCAT(COALESCE(CONCAT(c.supplier_business_name, '<br>'), ''), c.name) FROM transactions as ts 
                            JOIN contacts as c ON ts.contact_id=c.id
                            WHERE ts.id=t.id)
                        ) as customer"),
                'transaction_payments.amount',
    ...
    public function sellPaymentReport2(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    //     // Fetch payment types for the current business
    //     $payment_types = $this->transactionUtil->payment_types(null, true, $business_id);
    ...
    //         // Start query with necessary joins using Eloquent relationships and eager loading
    //         $query = TransactionPayment::with(['transaction', 'transaction.contact', 'transaction.contact.customerGroup'])
    ...
    //         // Filter by permitted locations for the user
    //         $permitted_locations = auth()->user()->permitted_locations();
    //         if ($permitted_locations !== 'all') {
    //             $query->whereIn('t.location_id', $permitted_locations);
    //         }
    ...
    //                     ? '<a data-href="' . action([\App\Http\Controllers\SellController::class, 'show'], [$row->transaction_id]) . '" href="#" data-container=".view_modal" class="btn-modal">' . $row->invoice_no . '</a>'
    //                     : '';
    //             })
    //             ->editColumn('paid_on', '{{@format_datetime($paid_on)}}')
    //             ->editColumn('method', function ($row) use ($payment_types) {
    ...
    //             })
    //             ->addColumn('action', function ($row) {
    //                 $view_button = '<button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-primary view_payment" data-href="' . action([\App\Http\Controllers\TransactionPaymentController::class, 'viewPayment'], [$row->DT_RowId]) . '">@lang("messages.view")</button>';
    //                 $download_button = !empty($row->document) ? '<a href="' . asset("/uploads/documents/" . $row->document) . '" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-accent" download=""><i class="fa fa-download"></i> @lang("purchase.download_document")</a>' : '';
    
    //                 return $view_button . ' ' . $download_button;
    //             })
    //             ->rawColumns(['invoice_no', 'amount', 'method', 'action', 'customer'])
    //             ->make(true);
    //     }
    
    //     // Render the initial view with necessary data
    //     $business_locations = BusinessLocation::forDropdown($business_id);
    //     $customers = Contact::customersDropdown($business_id, false);
    //     $customer_groups = CustomerGroup::forDropdown($business_id, false, true);
    
    //     return view('report.sell_payment_report')->with(compact('business_locations', 'customers', 'payment_types', 'customer_groups'));
    // }
