...
class ProductUtil extends Util
{
    ...
    public function updateProductQuantity($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true)
    {
        ...
        $business_locations = BusinessLocation::find($location_id);
        $company_branch = $business_locations->location_id;

        $business_id = $business_locations->business_id;
        ...
        if ($product->enable_stock == 1 && $qty_difference != 0) {
            //$this->api_notify_stock($product_id, $location_id, $qty_difference);
    ...
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0)
    {
        ...
        if ($product->enable_stock == 1) {
            //api_reduce_stock($product_id, $location_id, $qty_difference);
    ...
    /**
 * Adjusts stock over selling with purchases, opening stocks and stock transfers
...
public function adjustStockOverSelling($transaction)
{
    ...
    // Ensure purchase_lines relationship is loaded with product
    if (!$transaction->relationLoaded('purchase_lines')) {
        $transaction->load('purchase_lines.product');
    }

    foreach ($transaction->purchase_lines as $purchase_line) {
        try {
            // Skip if product doesn't exist or stock management is disabled
            if (empty($purchase_line->product) || $purchase_line->product->enable_stock != 1) {
                continue;
            }
            ...
            Log::error("Error processing purchase line ID {$purchase_line->id}: " . $e->getMessage());
            continue;
        }
    }
    ...
    public function api_notify_stock($product_id, $location_id, $amount){
        if($amount > 0){
            return api_add_stock($product_id, $location_id, abs($amount));
        }
        else{
            if($amount < 0){
                return api_reduce_stock($product_id, $location_id, abs($amount));
            }
            else{
                return null;
            }
