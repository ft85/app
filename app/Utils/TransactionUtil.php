...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $the_date=date("Ymd");
        $the_year=date("Y");
        $the_date_time=date("YmdHis");
        $obrdate=date("Y-m-d H:i:s");

       
        $business = Business::find($business_id);
        $company_tin = $business->tax_number_1;
        $company_name = $business->name;
        $company_phone=$business->mobile;
        
        $business_details_2 = $this->businessUtil->getDetails($business_id);
        $default_currency=$business_details_2->currency_code;
        $business_locations = BusinessLocation::find($input['location_id']);

        $company_branch = $business_locations->location_id;
        $contact = Contact::find($input['contact_id']);
        $custwalkingname = $contact->name;
        $contact_address=$contact->zip_code;

        $company_branch = $business_locations->location_id;
        $company_state=$business_locations->state;
        $company_address=$business_locations->zip_code;
        $tp_tradenumber = $business_locations->custom_field1;
        $commune = $business_locations->landmark;
        $quartier = $business_locations->city;
        $tp_activity_sector=$business_locations->custom_field2;
        $tp_legal_form = $business_locations->custom_field4;
        $tp_fiscalcentre = $business_locations->custom_field3;
        $tp_type = $business_locations->tp_type; 
        $tva_payer = $business_locations->tva_payer;
        $tp_address_number = $business_locations->tp_address_number;
        $payment_method_code = "01";
        $username = $business_locations->username;
        ...
        if ($contact->supplier_business_name == null) {
            ...
            $custNm = $contact->supplier_business_name;
            $custTin = $contact->tax_number;
            $custMblNo = $contact->mobile;
            $contact_address=$contact->contact_address;
        }

        $separator = '/';
          
        $newInvcNo=$invoice_no.'/'.$the_year;
        $total_tc=0;
        $newfinaltotal=0;
        ...
        if ($input['status'] != 'draft') {
            ...
            foreach ($input['products'] as $item) {
                ...
                $item_details = Product::find($item_id);
                $itemsname = $item_details->name;
                $itemcode = $item_details->injonge_code;
                $itemClsCd = $item_details->unspec;

                $qty = (float) str_replace(',', '', $item['quantity']);

                $price_with_comma = $item['unit_price_inc_tax'];
                $price_without_comma = str_replace(',', '', $price_with_comma);
                ...
                $item_tax = (float)$itemtax_without_comma;

                $item_price_without_tax=$price-$item_tax;

                $item_price_nvat = round($item_price_without_tax * $qty, 2);

                // $compound_tax=$item['optional_tax_id'];

                if (isset($item['optional_tax_id'])) {
                    $compound_tax = $item['optional_tax_id'];
                    $optional_tax_tax_details = TaxRate::find($compound_tax);
                    $taxname=$optional_tax_tax_details->name;
                    $taxrateamt=$optional_tax_tax_details->amount;
                } else {
                    $compound_tax = null; // Or any default value
                }

                

               // \Log::info("compound tax..".$compound_tax);

               // \Log::info(json_encode($optional_tax_tax_details));
        

                if ($taxname=='compound') {

                    
                    $vat=(round($item_price_without_tax*$qty,2)+($item_tax*$qty))*$taxrateamt/100;

                    $invoice_items_array[] = [
                    
                        'item_designation' =>$itemsname,
                        'item_price' =>round($item_price_without_tax,2),
                        'item_quantity'=>$qty,
                        'item_ct'=>$item_tax*$qty,
                        'item_tl'=>0,
                        'item_price_nvat'=>round($item_price_without_tax*$qty,2)+($item_tax*$qty),
                        'vat'=>$vat,   
                        'item_price_wvat'=>round($item_price_without_tax * $qty, 2)+($item_tax*$qty)+$vat,
                        'item_total_amount'=>round($item_price_without_tax * $qty, 2)+($item_tax*$qty)+$vat,
                    
                    ];

                   
                    $item['optional_item_tax']=$vat;  
                    
                    $total_tc=$total_tc+$vat;
                    $newfinaltotal=$newfinaltotal+round($item_price_without_tax * $qty, 2)+($item_tax*$qty)+$vat;

                 //   \Log::info("TC FOUND");
                 //   \Log::info(json_encode($invoice_items_array));
                 ...
                 //   \Log::info($taxname);
                  //  \Log::info($item_tax*$qty);
                    ...
                    $invoice_items_array[] = [
                        ...
                        'item_price_nvat'=>round($item_price_without_tax*$qty,2)+($item_tax*$qty),
                        'vat'=>0,   
                        'item_price_wvat'=>round($item_price_without_tax * $qty, 2)+($item_tax*$qty),
                        'item_total_amount'=>round($item_price_without_tax * $qty, 2)+($item_tax*$qty),
                    
                    ];

                  //  \Log::info("TC FOUND with no VAT ");
                  ...
                  //  \Log::info(json_encode($invoice_items_array));
                    ...
                    $invoice_items_array[] = [
                        ...
                        'vat'=>round($item_tax*$qty,2),   
                        'item_price_wvat'=>round($price * $qty, 2),
                        'item_total_amount'=>round($price * $qty, 2),
                    ...
                    //\Log::info('NO TC');
                    $newfinaltotal=$newfinaltotal+round($price * $qty, 2);
                ...
                if ($item_details->enable_stock == 1) {
                $obr_stock_movement=  [        
                    "system_or_device_id"=>$username,
                    "item_code"=>$itemcode,
                    "item_designation"=>$itemsname,
                    "item_quantity"=>$qty,
                    "item_measurement_unit"=>'Pieces',
                    "item_purchase_or_sale_price"=>$price,
                    "item_purchase_or_sale_currency"=>'BIF',
                    "item_cost_price"=>$price,
                    "item_cost_price_currency"=>$default_currency,
                    "item_movement_type"=>'SN',
                    "item_movement_description"=>'Normal Sale',
                    "item_movement_invoice_ref"=>$newInvcNo,
                    "item_movement_date"=>$obrdate,
        
                ];
                $obr_stock_movement_json=json_encode($obr_stock_movement);
                $obr_stock_movement_json= str_replace('\\', '', $obr_stock_movement_json);
        
                DB::table('stockmaster_obr')->insert([
            ['company_id' => $business_id, 'bhfid' => '00','request' =>$obr_stock_movement_json,'sent_obr' =>0,'url' =>'https://ebms.obr.gov.bi:8443/ebms_api/AddStockMovement/','created_at'=> now() ]]);        
        
                }

        }

        $invoice_total['tax'] = $total_tc;
        $invoice_total['final_total'] = $newfinaltotal;
        

        \Log::info('INVOICE NUMBER');
        \Log::info(json_encode($newInvcNo));
        

        $invoice_signature=$company_tin.'/'.$username.'/'.$the_date_time.'/'.$newInvcNo;
            $obr_request=  [
                'invoice_date'=>$obrdate,
                'invoice_type'=> 'FN',
                'invoice_number'=>$newInvcNo,
                'tp_type'=> $tp_type,
                'tp_trade_number'=>$company_name,
                'tp_name'=>$company_name,
                'tp_TIN' =>$company_tin,
                'tp_trade_number'=>$tp_tradenumber,
                'tp_postal_number'=>$tp_address_number,
                'tp_phone_number'=>$company_phone,
                'tp_address_province'=>$company_state,
                'tp_address_commune'=>$commune,
                'tp_address_quartier'=>$quartier,
                'tp_address_avenue'=>$company_address,
                'vat_taxpayer'=>$tva_payer,
                'ct_taxpayer'=>0,
                'tl_taxpayer'=>0,
                'tp_activity_sector'=>$tp_activity_sector,
                'tp_fiscal_center'=>$tp_fiscalcentre,
                ...
                'vat_customer_payer' =>1, //change this
                'cancelled_invoice_ref'=>'',
                'invoice_ref'=>$newInvcNo,
                'invoice_signature'=>$invoice_signature,
                'invoice_identifier'=>$invoice_signature,
                'invoice_signature_date'=>$obrdate,
                'invoice_items' =>$invoice_items_array
                
               ];
               
       // \Log::info(json_encode($obr_request));
        ...
        $transaction = Transaction::create([
            ...
            'request' => json_encode($obr_request),
            'rcptsign' => $invoice_signature,
    ...
    public function updateSellTransaction($transaction_id, $business_id, $input, $invoice_total, $user_id, $uf_data = true, $change_invoice_number = true)
    {
        ...
        $final_total = isset($input['final_total']) 
    ? ($uf_data ? $this->num_uf($input['final_total']) : $input['final_total']) 
    : 0; // Set a default value (0 or another suitable fallback)
        ...
        if ($input['status'] == 'final') {
            ...
            foreach ($input['products'] as $item) {
                ...
                $obr_stock_movement=  [        
                    ...
                    "item_measurement_unit"=>'Pieces',
                    "item_purchase_or_sale_price"=>$price,
                    "item_purchase_or_sale_currency"=>$default_currency,
                    "item_cost_price"=>$price,
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        //OBR information
        $output['registre_commerce'] = $location_details->custom_field1;
        $output['secteur'] = $location_details->custom_field2;
        $output['centre_fiscal'] = $location_details->custom_field3;
        $output['forme'] = $location_details->custom_field4;
        $output['tva_payer'] = $location_details->tva_payer;
        $output['tc_payer'] = $location_details->tc_payer;
        $output['pfa_payer'] = $location_details->pfa_payer;
        $output['rcptsign'] = $transaction->rcptsign;
        $output['signature'] = $transaction->signature;
        $output['customer_vat'] = $customer->custom_field1;
    ...
    public function getInvoiceNumber($business_id, $status, $location_id, $invoice_scheme_id = null, $sale_type = null)
    {
        if ($status == 'final') {
            ...
            \Log::info('scheme...'.json_encode($scheme));
    ...
    public function createSellReturnTransaction($business_id, $input, $invoice_total, $user_id)
    {
        $transaction = Transaction::create([
            ...
            'tax_id' => null,
            'discount_type' => $input['discount_type'],
            'discount_amount' => $this->num_uf($input['discount_amount'] ?? 0),
            'tax_amount' => $invoice_total['tax'],
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        if ((float) $sell->final_total == (float) $sell_return->final_total) {

            $obrcancelrequest=  [
                'invoice_signature'=>$sell->rcptsign,
                'cn_motif'=> 'Marchandise non conforme à la commande',
                
                 
                 ];
            
            $obrcancelrequest=json_encode($obrcancelrequest);
            $obrcancelrequest= str_replace('\\', '', $obrcancelrequest); 
          
            DB::table('stockmaster_obr')->insert([
                    ['company_id' => $business_id, 'bhfid' => '00','request' =>$obrcancelrequest,'sent_obr' =>0,'url' =>'https://ebms.obr.gov.bi:8443/ebms_api/cancelInvoice/','created_at'=> now() ]]);        
                


                    $business_locations = BusinessLocation::find($sell->location_id);
                    $username = $business_locations->username;
                    $obrdate=date("Y-m-d H:i:s");
                    foreach ($sell->sell_lines as $item) {
                        ...
                        $obr_stock_movement=  [        
                            ...
                            "item_movement_description"=>'Cancelled Invoice',
                            "item_movement_invoice_ref"=>$obrdate,
                            "item_movement_date"=>$obrdate,
        ...
        $company_branch = $business_locations->location_id;
        $contact = Contact::find($sell->contact_id);
        $custwalkingname = $contact->name;
        $contact_address=$contact->zip_code;
        ...
        $newInvcNo=$sell_return_data['invoice_no'].'/'.$the_year;
        ...
        foreach ($sell->sell_lines as $item) {
            ...
            $obr_stock_movement=  [        
                ...
                "item_cost_price_currency"=>'BIF',
                "item_movement_type"=>'ER',
                "item_movement_description"=>'Wrong Amounts',
                "item_movement_invoice_ref"=>$newInvcNo,
                "item_movement_date"=>$obrdate,
        ...
        $obr_request_credit=  [
            ...
            'cancelled_invoice'=>'N',
            'cancelled_invoice_ref'=>'',
            'cn_motif'=>'Wrong Amounts',
            'invoice_ref'=>$sell->invoice_no,
            'invoice_signature'=>$invoice_signature,
            'invoice_identifier'=>$invoice_signature,
            'invoice_signature_date'=>$obrdate,
            'invoice_items' =>$invoice_items_array
            ...
            $obr_request_credit=json_encode($obr_request_credit);
            $obr_request_credit= str_replace('\\', '', $obr_request_credit); 
          
            DB::table('stockmaster_obr')->insert([
                    ['company_id' => $business_id, 'bhfid' => '00','request' =>$obr_request_credit,'sent_obr' =>0,'url' =>'https://ebms.obr.gov.bi:8443/ebms_api/addInvoice_confirm/','created_at'=> now() ]]);        
