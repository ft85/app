...
class Util
{
    ...
    public function payment_types($location = null, $show_advance = false, $business_id = null)
    {
        ...
        // Fetch the business details based on the $business_id if provided
        if (!empty($business_id)) {
            $business = Business::find($business_id);
            $custom_labels = $business ? json_decode($business->custom_labels, true) : [];
        } else {
            $custom_labels = [];
        }

        // If location is provided, retrieve the business and custom labels
        if (!empty($location)) {
            $location = is_object($location) ? $location : BusinessLocation::find($location);
            if (empty($business) && !empty($location)) {
        ...
        // Add copay and covered payment methods if the business is a pharmacy
        if ($business && $business->is_pharmacy) {
            $payment_types['copay'] = __('Copay');
            $payment_types['covered'] = __('Covered');
        }
        ...
        if (!empty($location)) {
            ...
            // Initialize location account settings if empty
            if (empty($location_account_settings)) {
                $location_account_settings = [];
            }

            // Enable copay and covered methods for pharmacy-specific payment methods if the business is a pharmacy
            if ($business && $business->is_pharmacy) {
                if (!isset($location_account_settings['copay'])) {
                    $location_account_settings['copay'] = ['is_enabled' => 1];
                }
                if (!isset($location_account_settings['covered'])) {
                    $location_account_settings['covered'] = ['is_enabled' => 1];
                }

                // Save the updated settings back to the location
                $location->default_payment_accounts = json_encode($location_account_settings);
                $location->save();
            }
