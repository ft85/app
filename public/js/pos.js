...
$(document).ready(function() {
    ...
    $('select#customer_id')
        .select2({
            ...
            templateResult: function(data) {
                ...
                if (data.supplier_business_name) {
                    // If it's a business, display the business name
                    template.append($('<span>').text(data.supplier_business_name));
                } else if (data.text) {
                    // If it's an individual, display only the name part
                    var name = data.text.split('(')[0].trim();
                    template.append($('<span>').text('Name: ' + name));
                }

                // Add mobile number
                if (data.mobile) {
                    template.append($('<br>'));
                    template.append($('<span>').text(LANG.mobile + ': ' + data.mobile));
                }

                // Add tax number (TIN)
                if (data.tax_number) {
                    template.append($('<br>'));
                    template.append($('<strong>').text('TIN number: ' + data.tax_number));
                }
                ...
                if (typeof data.total_rp != 'undefined') {
                    ...
                    template.append(
                        $('<i>')
                            .addClass('fa fa-gift text-success')
                            .text(' ' + rp),
                    );
                }
            ...
            templateSelection: function(data) {
                if (data.supplier_business_name) {
                    return data.supplier_business_name;
                } else if (data.text) {
            ...
            language: {
                ...
                noResults: function() {
                    var $search = $('#customer_id').data('select2').dropdown.$search;
                    var name = $search.val();
        ...
        .on('select2:select', function(e) {
            ...
            var editButton = $('.edit_contact_button');
            var newUrl = '/contacts/' + data.id + '/edit';
            editButton.attr('href', newUrl);
        });
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        // Get the unit price including tax
        var unit_price_inc_tax = __read_number(tr.find('input.pos_unit_price_inc_tax'));
    
        // Calculate the initial line total without tax
        var line_total = entered_qty * unit_price_inc_tax;
    
        // Get the selected tax option
        var selectedTax = tr.find('select.optional_tax_id').find(':selected');

       // toastr.info('compound_tax_amount IS SELECTED ' + selectedTax);
    
        // If the selected tax is "compound", calculate and apply the compound tax
        if (selectedTax.length > 0 && selectedTax.text() === 'compound') {
            // Recalculate the line total with the compound tax
            var optional_tax_rate = selectedTax.data('rate'); // Get the tax rate from the dropdown
            var compound_tax_amount = (line_total * optional_tax_rate) / 100; // Calculate the compound tax
            line_total += compound_tax_amount; // Add compound tax to the line total

           // toastr.info('compound_tax_amount: ' + compound_tax_amount);
        ...
        // Change the modifier quantity text and hidden input values
        tr.find('.modifier_qty_text').each(function() {
            $(this).text(__currency_trans_from_en(entered_qty, false));
        });
        tr.find('.modifiers_quantity').each(function() {
        ...
        // Adjust combo quantity if needed
        adjustComboQty(tr);
    });
    ...
    $('button#pos-draft').click(function() {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                // Display primary success or error message
                if (result.success) {
                ...
                // Display print response message if available
                if (result.print_response) {
                    setTimeout(function() {
                        if (result.print_response.success) {
                            toastr.success(result.print_response.msg);
                        } else {
                            toastr.error(result.print_response.message);
                        }
                    }, 1000); // Delay the second notification to avoid overlap
                }
            },
            error: function(xhr, status, error) {
                enable_pos_form_actions();
                var response = JSON.parse(xhr.responseText);
                toastr.error(response.message ? response.message : 'An error occurred: ' + error);
            },
        });
    });
    ...
    $('button#pos-finalize').click(function() {
        ...
        $.ajax({
            url: '/get-customer-copay/' + selectedCustomerId,
            method: 'GET',
            success: function(response) {
                console.log('API Response:', response);

                if (response.copay !== undefined) {
                    document.getElementById('stored_copay').value = response.copay;
                    console.log('Copay set to:', response.copay);
                    console.log('Current input value:', document.getElementById('stored_copay').value);

                    // Show modal and trigger updateAmounts after setting copay
                    $('#modal_payment').modal('show').on('shown.bs.modal', function() {
                        updateAmounts(); // Recalculate amounts after modal is shown
                    });
                } else {
                    document.getElementById('stored_copay').value = 100;
                    console.log('No copay found, using default 100');
                    $('#modal_payment').modal('show');
                }
            },
            error: function(xhr) {
                console.error('Error fetching copay:', xhr);
                document.getElementById('stored_copay').value = 100;
                $('#modal_payment').modal('show');
            }
        });
    ...
    $('button#add-payment-row').off('click').click(function() {
...
// Add this script to your page or include it in your main JavaScript file
$(document).ready(function() {
    // Remove the href attribute from the edit button
    $('.edit_contact_button').removeAttr('href');

    // Add click event listener to the edit button
    $('.edit_contact_button').on('click', function(e) {
        e.preventDefault();
        ...
        if (selectedCustomerId) {
            // Make an AJAX request to get the customer data
            $.ajax({
                url: '/contacts/' + selectedCustomerId + '/edit',
                type: 'GET',
                dataType: 'html',
                success: function(response) {
                    // Assuming you have a modal with id 'editCustomerModal'
