$(document).ready(function() {
    ...
    //     var start = moment().subtract(29, 'days'); // Start date: 30 days ago
    //     var end = moment(); // End date: today
    
    //     // Extend your existing dateRangeSettings to include default start and end dates
    //     var dateRangeSettings = {
    //         startDate: start,  // Set default start date
    //         endDate: end,      // Set default end date
    //         locale: {
    //             format: moment_date_format,
    //             applyLabel: "{{ __('report.apply') }}",
    //             cancelLabel: "{{ __('report.cancel') }}",
    //             customRangeLabel: "{{ __('report.custom_range') }}"
    //         },
    //         ranges: {
    //             '{{ __("report.last_30_days") }}': [moment().subtract(29, 'days'), moment()],
    //             '{{ __("report.this_month") }}': [moment().startOf('month'), moment().endOf('month')],
    //             '{{ __("report.last_month") }}': [moment().subtract(1, 'month').startOf('month'), moment().subtract(1, 'month').endOf('month')],
    //             '{{ __("report.this_year") }}': [moment().startOf('year'), moment().endOf('year')],
    //         }
    //     };
    
    //     // Initialize daterangepicker with the settings
    //     $('#spr_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
    //         $('#spr_date_filter span').val(
    //             start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
    //         );
    ...
    //     // Set default value in input field to last 30 days when page loads
    //     $('#spr_date_filter').val(
    //         start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
    //     );
    
    //     // Reload data on date cancel
    //     $('#spr_date_filter').on('cancel.daterangepicker', function(ev, picker) {
    //         $('#spr_date_filter').val('');
    //         sell_payment_report.ajax.reload();
    //     });
