<div class="modal-dialog modal-lg" role="document">
          ...
          <div class="row">            
                  ...
                  <div class="form-group">
                      ...
                      <div class="input-group">
                          ...
                          {!! Form::text('tax_number', null, [
                              ...
                              'title' => __('The tax number must be exactly 9 digits.'),
                              'id' => 'tax_number'
                          ]); !!}
                          <span class="input-group-btn">
                              <button class="btn btn-primary" type="button" id="check_tax_number">{{ __('Verify') }}</button>
                          </span>
                      </div>
                      <span id="tax_number_error" style="color: red; display: none;">{{ __('The tax number must be exactly 9 digits.') }}</span>
                  </div>
              </div>
            ...
            <div class="form-group">
                   {!! Form::label('custom_field1', $contact_custom_field1 . ':') !!}
                      {!! Form::select('custom_field1', ['0' => 'Non', '1' => 'Oui'], null, ['class' => 'form-control', 'placeholder' => 'Select an option']) !!}
              </div>
            </div>
  ...
  {{-- <script>
      $(document).ready(function() {
          $('#check_tax_number').on('click', function() {
              var taxNumber = $('#tax_number').val();
              var taxNumberError = $('#tax_number_error');
  
              // Validate the tax number before making the API call
              
                  // Make the API call to verify the tax number
                  $.ajax({
                      url: '{{ route("verify.tax_number") }}',
                      method: 'POST',
                      data: {
                      ...
                      success: function(response) {
                          if (response.success) {
                              ...
                              $('#supplier_business_name').val(response.name); // Assuming response.data.name contains the name
                          } else {
                              alert(response.message);
                          }
                      },
                      error: function(xhr) {
                          alert('An error occurred while verifying the tax number.');
                          console.log(xhr);
                      }
                  });
  ...
  <script>
      $(document).ready(function() {
          // Set the "business" radio button to be checked by default
          $('#inlineRadio2').prop('checked', true);
  
          // Optional: Trigger any change events or actions related to selecting "business"
