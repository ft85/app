...
<script type="text/javascript">
    $(document).ready(function () {
        // Listen for changes in the optional tax dropdown
        $('table#pos_table tbody').on('change', 'select.optional_tax_id', function () {
           ...
           // toastr.info('changed: ');
            ...
            // Check if there is a selected tax option
            if (selectedTax.length === 0) {
                toastr.error('No tax option selected.');
                return; // Exit the function if no option is selected
            }

            // Check if the selected tax's name is "compound"
            if (selectedTax.text() === 'compound') {
                // Recalculate the line total with the compound tax
                updateLineTotalWithCompoundTax(tr);
            } else {
                // If not compound, recalculate without adding tax
                recalculateLineTotal(tr);
            }
        });
    });

	function updateLineTotalWithCompoundTax(tr) {
    // Calculate discounted unit price
    //var discounted_unit_price = calculate_discounted_unit_price(tr);

    // Get the tax rate from the selected optional tax dropdown
    var optional_tax_rate = tr.find('select.optional_tax_id').find(':selected').data('rate');
	...
	toastr.info('line_total: ' + line_total);
...
// Function to determine if the selected tax is compound
function isCompoundTax(taxId) {
    // Logic to check if the taxId corresponds to a compound tax
    // Replace this with your actual logic
    return taxId === 'compound_tax_id'; // Adjust this based on your tax IDs
}
