...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	<td>
		...
		<span class="text-link text-info cursor-pointer" data-toggle="modal" data-target="#row_edit_product_price_modal_{{$row_count}}">
			{!! $product_name !!}
			&nbsp;<i class="fa fa-info<br>-circle"></i>
		</span>
		</div>
		@else
			{!! $product_name !!}
	...
	@if(!empty($is_direct_sell))
		...
		<td class="text-center {{$hide_tax}}">
			...
			{!! Form::hidden("products[$row_count][optional_item_tax]", @num_format($optional_item_tax ?? 0), ['class' => 'optional_item_tax']); !!}
    
			{{-- Dropdown for the second (optional) tax selection with no default selection --}}
			{!! Form::select("products[$row_count][optional_tax_id]", $tax_dropdown['tax_rates'], null, ['placeholder' => 'Optional', 'class' => 'form-control optional_tax_id'], $tax_dropdown['attributes']); !!}
