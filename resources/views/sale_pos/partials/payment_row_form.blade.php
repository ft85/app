<div class="row">
    ...
    @php
        $isPharmagy = session()->get('business.is_pharmacy');
        $payment_rows = [
            [
                'name' => 'Copay',
                'method' => 'copay'
    ...
    @endphp
    @if($isPharmagy)
        @foreach($payment_rows as $index => $default_payment)
            <div class="payment-row">
                <div class="{{ $col_class }}">
                    <div class="form-group">
                        {!! Form::label("amount_$index", $default_payment['name'] . '(%):*', ['id' => $default_payment['method'] . '_label']) !!}
                        <div class="input-group">
                            <span class="input-group-addon">
                                <i class="fas fa-money-bill-alt"></i>
                            </span>
                            {!! Form::text("payment[$index][amount]", @num_format($payment_line['amount']), ['class' => 'form-control payment-amount input_number', 'required', 'id' => "amount_$index", 'placeholder' => __('sale.amount'), 'readonly']); !!}
                        </div>
                    </div>
                </div>
                ...
                @if(!empty($show_date))
                    <div class="{{$col_class}}">
                        <div class="form-group">
                            {!! Form::label("paid_on_$index", __('lang_v1.paid_on') . ':*') !!}
                            <div class="input-group">
                        <span class="input-group-addon">
                            <i class="fa fa-calendar"></i>
                        </span>
                                {!! Form::text("payment[$index][paid_on]", isset($payment_line['paid_on']) ? @format_datetime($payment_line['paid_on']) : @format_datetime('now'), ['class' => 'form-control paid_on', 'readonly', 'required']); !!}
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <div class="{{$col_class}}">
                <div class="form-group">
                    {!! Form::label("method_$index" , __('lang_v1.payment_method') . ':*') !!}
                    <div class="input-group">
                    <span class="input-group-addon">
                        <i class="fas fa-money-bill-alt"></i>
                    </span>
                        {!! Form::select("payment[$index][method]", $payment_types, $default_payment['method'], ['class' => 'form-control col-md-12 payment_types_dropdown', 'required', 'id' => !$readonly ? "method_$index" : "method_advance_$index", 'style' => 'width:100%;', 'disabled' => $readonly]); !!}
                    </div>
                </div>
            </div>
                ...
                @if(!empty($accounts))
                    <div class="{{$col_class}}">
                        <div class="form-group">
                            {!! Form::label("account_$index", __('lang_v1.payment_account') . ':') !!}
                            <div class="input-group">
                        <span class="input-group-addon">
                            <i class="fas fa-money-bill-alt"></i>
                        </span>
                                {!! Form::select("payment[$index][account_id]", $accounts, !empty($payment_line['account_id']) ? $payment_line['account_id'] : '', ['class' => 'form-control select2 account-dropdown', 'id' => "account_$index", 'style' => 'width:100%;', 'disabled' => true]); !!}
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <div class="col-md-12">
                    <div class="form-group">
                        {!! Form::label("note_$index", __('sale.payment_note') . ':') !!}
                        {!! Form::textarea("payment[$index][note]", $default_payment['name'] . ' payment', ['class' => 'form-control', 'rows' => 3, 'id' => "note_$index", 'readonly' => true]); !!}
                    </div>
                </div>
