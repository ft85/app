    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input type="hidden" id="stored_copay" value="100">
                <div class="row">
                    <div class="col-md-12 mb-12">
                        <strong>@lang('lang_v1.payment'):</strong> <span id="advance_balance_text"></span>
...
<div class="modal fade" tabindex="-1" role="dialog" id="card_details_modal">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <!-- Add this right after <div class="modal-body"> in your payment modal -->
    ...
    $(document).ready(function() {
        ...
        function updateAmounts() {
            if (initialTotal === null) {
                var rawValue = $('.payment-amount').first().val() || '0';
                var cleanValue = rawValue.replace(/,/g, '');
                initialTotal = parseFloat(cleanValue);
            }

            var copayPercentage = parseFloat(document.getElementById('stored_copay').value || 100);
            console.log('Using copay percentage:', copayPercentage);
            console.log('Initial total amount:', initialTotal);

            // Check if the business is a pharmacy
            var isPharmacy = "<?php echo session()->get('business.is_pharmacy') ? 'true' : 'false'; ?>";
            console.log('Is Pharmacy:', isPharmacy);

            if (isPharmacy) {
                if (!isNaN(initialTotal) && initialTotal > 0) {
                    var copayAmount = (initialTotal * (copayPercentage / 100)).toFixed(2);
                    $('#amount_0').val(copayAmount);
                    // Update copay label with actual percentage
                    $('#copay_label').text('Copay (' + copayPercentage.toFixed(1) + '%):*');
                    console.log('Copay amount:', copayAmount);

                    var coveredPercentage = (100 - copayPercentage);
                    var coveredAmount = (initialTotal * (coveredPercentage / 100)).toFixed(2);
                    $('#amount_1').val(coveredAmount);
                    // Update covered label with actual percentage
                    $('#covered_label').text('Covered (' + coveredPercentage.toFixed(1) + '%):*');
                    console.log('Covered amount:', coveredAmount);
                } else {
                    $('#amount_0').val('');
                    $('#amount_1').val('');
                    // Reset labels to default
                    $('#copay_label').text('Copay (%):*');
                    $('#covered_label').text('Covered (%):*');
                    console.log('Invalid total, resetting amounts');
                }
            } else {
                $('#amount_0').val(initialTotal);
                $('#amount_1').val('');
                // Reset labels to default
                $('#copay_label').text('Copay (%):*');
                $('#covered_label').text('Covered (%):*');
        ...
        $('#modal_payment').on('shown.bs.modal', function() {
            console.log('Modal opened, initializing calculations');
            updateAmounts();
        });

        $('.payment-amount').first().on('input', function() {
            initialTotal = parseFloat($(this).val() || 0);
            console.log('Total amount changed:', initialTotal);
            updateAmounts();
        });
    });
</script> -->
...
<script>
    $(document).ready(function() {
        // Initialize select2 and datetime picker
        $('.select2').select2();
        $('.paid_on').datetimepicker({
            format: moment_date_format + ' ' + moment_time_format,
        });

        // Variables to track initial total
        var initialTotal = null;

        // Function to update copay and covered amounts
        function updateAmounts() {
            // If initialTotal is null, initialize it from the first payment amount input
            if (initialTotal === null) {
                var rawValue = $('.payment-amount').first().val() || '0';
                var cleanValue = rawValue.replace(/,/g, '');
                initialTotal = parseFloat(cleanValue);
            ...
            // Get copay percentage (defaults to 100 if not set)
            var copayPercentage = parseFloat(document.getElementById('stored_copay').value || 100);
            console.log('Using copay percentage:', copayPercentage);
            console.log('Initial total amount:', initialTotal);

            // Check if the business is a pharmacy and convert to boolean
            var isPharmacy = "<?php echo session()->get('business.is_pharmacy') ? 'true' : 'false'; ?>" === 'true';
            console.log('Is Pharmacy:', isPharmacy);

            if (isPharmacy) {
                if (!isNaN(initialTotal) && initialTotal > 0) {
                    // Calculate copay and covered amounts based on percentage
                    var copayAmount = (initialTotal * (copayPercentage / 100)).toFixed(2);
                    var coveredAmount = (initialTotal * ((100 - copayPercentage) / 100)).toFixed(2);
                    
                    $('#amount_0').val(copayAmount);
                    $('#amount_1').val(coveredAmount);

                    // Update labels with actual percentage values
                    $('#copay_label').text('Copay (' + copayPercentage.toFixed(1) + '%):*');
                    $('#covered_label').text('Covered (' + (100 - copayPercentage).toFixed(1) + '%):*');
                    
                    console.log('Copay amount:', copayAmount);
                    console.log('Covered amount:', coveredAmount);
                } else {
                    // Clear values and reset labels if initial total is invalid
                    $('#amount_0').val('');
                    $('#amount_1').val('');
                    $('#copay_label').text('Copay (%):*');
                    $('#covered_label').text('Covered (%):*');
                ...
                // If not a pharmacy, set initial total as copay and clear covered amount
                $('#amount_0').val(initialTotal);
                $('#amount_1').val('');
                $('#copay_label').text('Copay (%):*');
                $('#covered_label').text('Covered (%):*');
        ...
        // Trigger updateAmounts when modal is opened
        $('#modal_payment').on('shown.bs.modal', function() {
            console.log('Modal opened, initializing calculations');
            updateAmounts();
        });

        // Update initialTotal dynamically when payment-amount input changes
        $('.payment-amount').first().on('input', function() {
            var rawValue = $(this).val() || '0';
            initialTotal = parseFloat(rawValue.replace(/,/g, '')) || 0;
            console.log('Total amount changed:', initialTotal);
            updateAmounts();
        });
    });
